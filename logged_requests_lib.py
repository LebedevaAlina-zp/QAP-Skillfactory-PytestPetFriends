        ...
        with open(log_filename, "a+") as log_file:
            ...
            try:
                for i in range(1, len(args_values)):
                    ...
                    try:
                        log_file.write(f"{(args_repr + kwargs_repr)[i]}\n")
                    except UnicodeEncodeError:
                        log_file.write(f"{(args_repr + kwargs_repr)[i].encode('utf-8')}\n")
                ...
                log_file.write(f"{indent}"+ f"{args_names_list}\n" + f"{args_values.encode('utf-8')}")
